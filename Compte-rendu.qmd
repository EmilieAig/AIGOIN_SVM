...
from scripts_Python.svm_source import *
...
Maintenant que nos données sont correctement mélangées et séparées aléatoirement, nous allons pouvoir évaluer la performance en généralisation du modèle à partir d'un \textbf{noyau linéaire}. Ce noyau permet de chercher une frontière de décision linéaire (droite en $2$D).
...
Dans cette question, nous nous basons sur une [application](https://scikit-learn.org/1.2/auto_examples/applications/svm_gui.html) qui permet, en temps réel, d'évaluer l'impact du choix du noyau et du paramètre de régularisation $C$.
...
Dans cette partie, nous utilisons une [base de données](https://scikit-learn.org/0.19/datasets/labeled_faces.html) extraite de "Labeled Faces in the Wild" afin d'exposer un problème de classification de visages.
...
```{python}
# Code supplémentaire de vérification/interprétation

# Vérifier la distribution des classes
unique, counts = np.unique(y, return_counts=True)
class_distrib = dict(zip(unique, counts))

# Affichage des résultats
print("Distribution des classes :")
print(f"{names[0]} : {class_distrib[0]} images ({class_distrib[0]/len(y)*100:.1f}%)")
print(f"{names[1]} : {class_distrib[1]} images ({class_distrib[1]/len(y)*100:.1f}%)")
...
On peut voir dans la [Figure 3](#fig-cOpti) qu'il y a $92,7\%$ de précision sur l'ensemble de test, ce qui est une très bonne performance pour la classification de nos visages. Cela signifie que le modèle prédit correctement l'identité de la personne dans $93\%$ des cas.
...
La [galerie d'images](#fig-heatmap) permet d'évaluer visuellement les performances du modèle. On peut voir que la majorité des visages sont correctement classés (seulement $2$ erreurs sur les prédictions de Powell, ce qui correspond ici à environ $83\%$ de prédiction correcte). On voit également que le modèle a réussi à identifier les personnes même avec des variations comme les expressions faciales, les angles de vue, les conditions d'éclairage ou encore la présence/absence de lunettes.
...
- Concentration de l'information : les $n$ premières composantes capturent un certain pourcentage de la variance totale (nous allons le voir par la suite dans le [tableau récapitulatif](#tableau)). Cette variance correspond principalement aux vraies caractéristiques faciales. Ainsi, on passe de plus de $400$ dimension à $20$, donc le ratio entre le signal et le bruit est nettement amélioré.
...
| 30                    | 0,691       | 0,603      | 8,8%  |
| 50                    | 0,770       | 0,536      | 23,4% |
| 100                   | 0,888       | 0,542      | 34,6% |
| 200                   | 0,966       | 0,542      | 42,1% |
...
Ensuite, afin de réaliser la [partie 3](#part3), nous nous sommes servis du code disponible via [ce lien](https://scikit-learn.org/1.2/auto_examples/applications/svm_gui.html) et écrit par P. Prettenhoer. En ce qui concerne la [partie 4](#part4), nous nous sommes servis de la base de données disponible via [ce lien](https://scikit-learn.org/0.19/datasets/labeled_faces.html). 
