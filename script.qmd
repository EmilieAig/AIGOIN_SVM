...
Au niveau du lancement du code, le temps de calcul s'est révélé particulièrement long pour cette expérimentation. L'exécution complète a nécessité environ $20$ minutes pour toutes les valeurs de $n$ testées, à l'exception notable de $n=100$ et $n=200$ qui ont été nettement plus rapides. Cette différence s'explique par le fait que pour les faibles valeurs de $n$ ($≤ 30$), l'algorithme d'optimisation du SVM doit explorer un espace de solutions plus contraint avec moins de degrés de liberté, ce qui peut paradoxalement ralentir la convergence. À l'inverse, pour $n=100$ et $n=200$, bien que le nombre de composante soit plus élevée, l'algorithme converge plus rapidement car les données deviennent linéairement séparables dans cet espace de grande dimension (au prix d'un overfitting massif).
